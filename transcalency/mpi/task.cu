...
void shareLimits(double* A, Context ctx)
{
    ...
    {
        MPI_Send(&A[(ctx.height.self-2)*ctx.netSize],ctx.netSize,MPI_DOUBLE,ctx.rank+1,0,MPI_COMM_WORLD);
        MPI_Recv(&A[(ctx.height.self-1)*ctx.netSize],ctx.netSize,MPI_DOUBLE,ctx.rank+1,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
    if (ctx.rank!=0)
    {
        MPI_Send(&A[ctx.netSize],ctx.netSize,MPI_DOUBLE,ctx.rank-1,0,MPI_COMM_WORLD);
...
void checkAccuracy(double* Anew, double* A, double* Delta, Context ctx, double accuracy, double* loss, bool* running, void *d_temp_storage, size_t temp_storage_bytes, double* val)
{
    ...
    cudaMemcpy(loss, val, sizeof(double), cudaMemcpyDeviceToHost);

    if(ctx.rank != 0){
        MPI_Send(loss, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
...
Context getContext(int rank, int count, int netSize){
    ...
    return {rank, count, netSize, size, height};
}
